---
title: "Gb_20220908"
date: 2022-10-29T20:28:25+08:00
lastmod: 2022-10-29
...
.chart { margin-left: 1.5rem; margin-right: 1.5rem; width: 800px; } .quote { margin-left: 2rem; margin-right: 2rem; color: #999; font-style: italic; font-size: 120%; } @media (prefers-color-scheme: dark) { .chart { border-radius: 8px; } }
...
After Go 1.18 was released with support for type parameters (more commonly referred to as _generics_), we wanted to understand what the initial awareness and adoption of generics looked like, as well as identify common challenges or blockers for using generics.

The vast majority of survey respondents (86%) were already aware generics shipped as part of the Go 1.18 release. We had hoped to see a simple majority here, so this was much more awareness than we’d been expecting. We also found that about a quarter of respondents had begun using generics in Go code (26%), including 14% who said they are already using generics in production or released code. A majority of respondents (54%) were not opposed to using generics, but didn’t have a need for them today. We also found that 8% of respondents _wanted_ to use generics in Go, but were currently blocked by something.

![Chart showing most
respondents were aware Go 1.18 included generics](survey2022q2/generics_awareness.svg) ![Chart showing 26% of respondents are
already using Go generics](survey2022q2/generics_use.svg)
...
generic challenges](survey2022q2/text_gen_challenge.svg)
...
implementation](survey2022q2/text_gen_feedback.svg)
...
common security challenge is evaluating the security of third-party libraries](survey2022q2/security_challenges.svg)
...
not tried fuzz testing yet](survey2022q2/fuzz_use.svg) ![Chart showing the biggest fuzz
testing challenges relate to understanding, rather than technical issues](survey2022q2/text_fuzz_challenge.svg)
...
respondents have not found security vulnerabilities during the past year](survey2022q2/security_found_vuln.svg) ![Chart showing
that vulnerability scanners are the most common way to learn about security
vulnerabilities](survey2022q2/text_vuln_find.svg)
...
![Chart showing most
respondents resolved vulnerabilities by upgrading dependencies](survey2022q2/text_vuln_resolve.svg) ![Chart showing a 6-way
tie for tasks that were most challenging when investigating and resolving
security vulnerabilities](survey2022q2/text_vuln_challenge.svg)
...
Following the 2021 switch to [power VS Code’s Go support via the gopls language server](https://go.dev/blog/gopls-vscode-go), the Go team has been interested in understanding developer pain points related to gopls. While we receive a healthy amount of feedback from developers currently using gopls, we wondered whether a large proportion of developers had disabled it shortly after release, which could mean we weren’t hearing feedback about particularly problematic use cases. To answer this question, we asked respondents who said they preferred an editor which supports gopls whether or not they _used_ gopls, finding that only 2% said they had disabled it; for VS Code specifically, this dropped to 1%. This increases our confidence that we’re hearing feedback from a representative group of developers. For readers who still have unresolved issues with gopls, please let us know by [filing an issue on GitHub](https://github.com/golang/go/issues).

![Chart showing the top
preferred editors for Go are VS Code, GoLand, and Vim / Neovim](survey2022q2/editor_self_select.svg) ![Chart showing only 2% of
respondents disabled gopls](survey2022q2/use_gopls.svg)
...
survey](survey2022q2/workspaces_use_s.svg) ![Chart showing that documentation and error messages were the top
challenge when trying to use Go workspaces](survey2022q2/text_workspace_challenge.svg)
...
![Chart showing most
respondents use source code directly for internal package documentation](survey2022q2/doc_viewing_today.svg) ![Chart
showing 39% of respondents would prefer to use a documentation server instead
of viewing source for docs](survey2022q2/doc_viewing_ideal.svg) ![Chart showing most respondents
expect a software engineer to be responsible for such a documentation server](survey2022q2/doc_server_owner.svg)
...
respondents' Go experience](survey2022q2/go_exp.svg) ![Chart showing distribution of where respondents' use Go](survey2022q2/where.svg) ![Chart showing distribution of
organization sizes for survey respondents](survey2022q2/org_size.svg) ![Chart showing distribution of industry
classifications for survey respondents](survey2022q2/industry.svg) ![Chart showing where in the world survey
respondents live](survey2022q2/location_s.svg)
...
respondents build with Go](survey2022q2/go_app.svg) ![Chart showing Linux and macOS are the most common development systems](survey2022q2/os_dev.svg) ![Chart showing
Linux is the most common deployment platform](survey2022q2/os_deploy.svg)
...
are satisfied using Go, with 4% dissatisfied](survey2022q2/csat.svg) ![Chart showing a long tail
of challenges reported by survey respondents](survey2022q2/text_biggest_challenge.svg)
