...
⚫ Redis 集群各方法的响应时间均为最低。随着并发量和业务数量的ᨀ升其响应时间会有明显上升（公有集群影响因素偏大），但是极限 QPS 可以达到最大且基本无异常。
...
⚫ ZK 具有较好的稳定性；响应时间抖动很小，没有出现异常。但是随着并发量和业务数量的ᨀ升其响应时间和 QPS 会明显下降。
...
Chubby 对外ᨀ供类似于文件系统的 API，在 Chubby 创建文件路径即加锁操作。
...
RheaKV 对外ᨀ供 lock 接口，为了优化数据的读写，按不同的存储类型，ᨀ供不同的锁特性。RheaKV ᨀ供 wathcdog 调度器来控制锁的自动续租机制，避免锁在任务完成前ᨀ前释放，和锁永不释放造成死锁。
...
⚫ Availability（可用性） 指系统ᨀ供的服务必须一直处于可用的状态，每次请求都能获取到非错的响应（不保证获取的数据为最新数据）
⚫ Partition tolerance（分区容错性） 分布式系统在遇到任何网络分区故障的时候，仍然能够对外ᨀ供满足一致性和可用性的服务，除非整个网络环境都发生了故障
...
当发生网络分区的时候，如果我们要继续服务，那么强一致性和可用性只能 2选 1。也就是说当网络分区之后 P 是前ᨀ，决定了 P 之后才有 C 和 A 的选择。也就是说分区容错性（Partition tolerance）我们是必须要实现的。

#### 11、请讲一下你对 BASE 理论的理解
BASE 理论由 eBay 架构师 Dan Pritchett ᨀ出，在 2008 年上被分表为论文，并且 eBay 给出了他们在实践中总结的基于 BASE 理论的一套新的分布式事务解决方案。
...
针对数据库领域，BASE 思想的主要实现是对业务数据进行拆分，让不同的数据分布在不同的机器上，以ᨀ升系统的可用性，当前主要有以下两种做法：
⚫ 按功能划分数据库
⚫ 分片（如开源的 MyCat、Amoeba 等）。
#### 12、分布式与集群的区别是什么？
分布式：一个业务分拆多个子业务，部署在不同的服务器上

集群：同一个业务，部署在多个服务器上。比如之前做电商网站搭的 redis 集群以及 solr 集群都是属于将 Redis 服务器ᨀ供的缓存服务以及 solr 服务器ᨀ供的搜索服务部署在多个服务器上以ᨀ高系统性能、并发量解决海量存储问题。
#### 13、请讲一下 BASE 理论的三要素
基本可用
...
#### 14、请说一下对两阶段提交协议的理解
分布式系统的一个难点是如何保证架构下多个节点在进行事务性操作的时候保持一致性。为实现这个目的，二阶段ᨀ交算法的成立基于以下假设：
...
⚫ 协调者节点向所有参与者节点询问是否可以执行ᨀ交操作(vote)，并开始等待各参与者节点的响应。
...
⚫ 协调者节点向所有参与者节点发出”正式ᨀ交(commit)”的请求。
